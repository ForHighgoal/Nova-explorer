<template>
    <div class="address-asset-list-dt">
        <f-data-table :columns="columns" :items="items" first-m-v-column-width="6" class="f-data-table-body-bg-color">
            ...
            <template v-slot:column-available="{ value, item, column }">
                <div v-if="column" class="row no-collapse no-vert-col-padding">
                    <div class="col-6 f-row-label">{{ column.label }}</div>
                    <div class="col break-word">
                        <f-token-value :value="value" :token="item" :use-placeholder="false"
                            :max-decimals="MAX_TOKEN_DECIMALS_IN_TABLES" no-currency />
                    </div>
                </div>
                <template v-else>
                    <f-token-value :value="value" :token="item" :use-placeholder="false"
...
export default {
    ...
    watch: {
         ...
         */
        tokens(_value) {
            // this.items = _value.filter((_item) => _item.isActive && _item.canDeposit && _item.symbol !== 'NOVA');
    ...
    methods: {
         ...
         */
        usedInFMint(_token) {
            return _token.symbol === 'WNOVA' || _token.symbol === 'FUSD';
        },
    },
};
</script>
