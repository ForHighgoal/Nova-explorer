<template>
    <div class="hometransactionlist">
        <transition enter-active-class="fadelong-enter-active">
            <f-data-table v-show="show" :columns="dColumns" :hidden-columns="hiddenColumns" :items="dItems"
                :loading="cLoading" f-card-off mobile-view height="300px" first-m-v-column-width="5" fixed-header
                v-bind="{ ...$attrs, ...$props }" class="f-data-table-body-bg-color">
                ...
                <template v-slot:column-address="{ value, column }">
                    <div v-if="column" class="row no-collapse no-vert-col-padding">
                        <div class="col-5 f-row-label">{{ column.label }}</div>
                        <div class="col"><router-link :to="{ name: 'address-detail', params: { id: value } }" :title="value">{{
