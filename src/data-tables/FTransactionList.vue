<template>
    <div class="transaction-list-dt">
        <template v-if="!dTransactionListError">
            <f-data-table :columns="dColumns" :hidden-columns="hiddenColumns" :items="dItems" :mobile-view="cMobileView"
                :disable-infinite-scroll="!dHasNext" :loading="cLoading" :f-card-off="fCardOff" first-m-v-column-width="5"
                infinite-scroll fixed-header v-bind="{ ...$attrs, ...$props }" class="f-data-table-body-bg-color"
...
-->
                ...
                <template v-slot:column-hash="{ value, column }">
                    <div v-if="column" class="row no-collapse no-vert-col-padding">
                        ...
                        <div class="col">
                            <router-link :to="{ name: 'transaction-detail', params: { id: value } }" :title="value">{{ value
                                                            |
                                                            formatHash }}</router-link>
                        </div>
                    </div>
                    <template v-else>
                        <router-link :to="{ name: 'transaction-detail', params: { id: value } }" :title="value">{{ value |
...
-->
                ...
                <template v-slot:column-timestamp="{ value, column }">
                    <div v-if="column" class="row no-collapse no-vert-col-padding">
                        <div class="col-5 f-row-label">{{ column.label }}</div>
                        <div class="col">
                            <timeago :datetime="timestampToDate(value)" :auto-update="1"
                ...
                <template v-slot:column-address="{ value, column }">
                    <div v-if="column" class="row no-collapse no-vert-col-padding">
                        <div class="col-5 f-row-label">{{ column.label }}</div>
                        <div class="col"><router-link :to="{ name: 'address-detail', params: { id: value } }"
                                :title="value">{{
                                                                value | formatHash }}</router-link></div>
                    </div>
                    <template v-else>
                        <router-link :to="{ name: 'address-detail', params: { id: value } }" :title="value">{{ value |
                                                    formatHash }}</router-link>
                    </template>
                </template>
                    ...
                    <div v-if="column" class="row no-collapse no-vert-col-padding">
                        ...
                        <div class="col">
                            <template v-if="addressCol">
                                <f-account-transaction-amount :address="addressCol" :from="getFrom(item)" :to="getTo(item)"
                                    :amount="value" />
                            </template>
                            <template v-else>
        ...
        return {
            ...
            dColumns: [
                ...
                */
                ...
                {
                    name: 'amount',
                    label: `${this.$t('view_transaction_list.amount')} (NOVA)`,
                ...
                /*
                                    {
                                        name: 'fee',
                                        label: `${this.$t('view_transaction_list.fee')} (NOVA)`,
                                        itemProp: 'gasUsed',
                                        width: '130px'
                                    }
                */
